...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		CreateWindow(L"edit", NULL, WS_CHILD | WS_VISIBLE | WS_BORDER,
			0, 35, 100, 25, hWnd, (HMENU)3002, hInst, NULL);
		CreateWindow(L"button", L"OK", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
			120, 35, 100, 25, hWnd, (HMENU)4002, hInst, NULL);
    ...
    case WM_COMMAND:
        {
			...
			case 4001:
			{
				...
				if (nNum >= 10 && nNum <= 20) {
					...
					swprintf_s(szBuf, 256, L"%d는 10에서 20 사이의 값입니다.", nNum);
					TextOut(hdc, 0, 120, szBuf, wcslen(szBuf));

				}
				else {
					swprintf_s(szBuf, 256, L"%d는 10에서 20 사이의 값이 아닙니다.", nNum);
					TextOut(hdc, 0, 120, szBuf, wcslen(szBuf));
				}
				ReleaseDC(hWnd, hdc);
			}
			...
			case 4002:
			{
				HDC hdc = GetDC(hWnd);
				TCHAR szBuf[256];
				//HWND hEdit = GetDlgItem(hWnd,3002); 
				...
				if (nNum != 0) {
					...
					swprintf_s(szBuf, 256, L"%d은 0이 아니므로 정상 이용하실 수 있습니다.", nNum);
					TextOut(hdc, 0, 150, szBuf, wcslen(szBuf));

				}
				else {
					swprintf_s(szBuf, 256, L"%d은 0이 아니어야만 합니다.", nNum);
					TextOut(hdc, 0, 150, szBuf, wcslen(szBuf));
				}
				ReleaseDC(hWnd, hdc);
			}
