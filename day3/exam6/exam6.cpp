    ...
    case WM_COMMAND:
        {
			...
			case ID_DRAWING_LINEAR:
			{
				...
				myPen = CreatePen(PS_SOLID, rand() % 30, RGB(rand() % 255, rand() % 255, rand() % 255));
				...
				break;
			case ID_DRAWING_SQUARE:
			{
				int xPos = rand() % 300;
				int yPos = rand() % 300;
				int xSize = 0 + (rand() % 190);
				int ySize = 0 + (rand() % 190);
			...
			case ID_DRAWING_CIRCLE:
			{
				...
				Ellipse(hDC, xPos, yPos, xPos + xSize, yPos + ySize);
